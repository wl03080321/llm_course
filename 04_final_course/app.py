    ...
    if new_uploaded_files:
        ...
        if results["failed_files"]:
            ...
            for failed in results["failed_files"]:
                final_result.append(
                    f"  ✗ {failed['filename']} - 錯誤: {failed['error']}"
                )
...
def chatbot_reply(message, history, system_prompt, max_token, temperature):
    ...
    try:
        ...
        if actual_files:
            ...
            retrieved_docs = rag_system.query_and_retrieve(
                message, k=3, return_scores=True
            )
...
with gr.Blocks(title="RAG ChatBot System") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            chatbot = gr.ChatInterface(
                fn=chatbot_reply,
                additional_inputs=[system_prompt, max_token, temperature],
                title="🤖 智能問答助手",
        ...
        with gr.Column(scale=1):
            ...
            with gr.Group():
                ...
                with gr.Row():
                    delete_selected_btn = gr.Button(
                        "🗑️ 刪除選中檔案", variant="secondary", size="sm"
                    )

                delete_result = gr.Textbox(label="刪除結果", interactive=False, lines=3)

            # 向量資料庫資訊
            with gr.Group():
                gr.Markdown("#### 📊 向量資料庫狀態")
                db_info = gr.Textbox(label="資料庫資訊", interactive=False, lines=5)
                file_details = gr.Textbox(
                    label="檔案詳細資訊", interactive=False, lines=4
                )
            ...
            file_upload.change(
                ...
                fn=lambda: gr.CheckboxGroup(
                    choices=get_uploaded_files_choices(), value=[]
                ),
            ...
            def delete_files_and_update(selected_files):
                ...
                return result_message, gr.CheckboxGroup(
                    choices=updated_choices, value=[]
                )
            ...
            ).then(fn=update_status, outputs=[db_info]).then(
                fn=get_file_details, outputs=file_details
            )
            ...
            refresh_btn.click(fn=refresh_info, outputs=[db_info, file_details])
            # 清除檔案按鈕
            with gr.Row():
                clear_files_btn = gr.Button(
                    "🗑️ 清除所有檔案", variant="secondary", size="sm"
                )
    ...
    def init_status():
        ...
        return (
            db_text,
            file_details_text,
            gr.CheckboxGroup(choices=file_choices, value=[]),
        )

    demo.load(fn=init_status, outputs=[db_info, file_details, files_to_delete])

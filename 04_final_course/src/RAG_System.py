...
class RAGSystem:
    ...
    def _load_embedding_model(self):
        ...
        embedding_model = load_embedding_model(
            model_name=self.embedding_model_name, cache_folder=self.model_path
        )
    ...
    def get_documents(
        self, filter_dict: Optional[Dict] = None, limit: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        從向量資料庫獲取文件
    ...
    def delete_documents_by_filename(self, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result["message"] = (
                f"成功刪除檔案 '{filename}' 的 {len(docs['ids'])} 個文件"
            )
        ...
        for filename in uploaded_files:
            ...
            if not os.path.exists(file_path):
                ...
                results["failed_files"].append(
                    {"filename": filename, "error": "File not found"}
                )
                continue
            ...
            try:
                ...
                if file_extension == ".pdf":
                    ...
                    if docs:
                        ...
                        results["failed_files"].append(
                            {"filename": filename, "error": "PDF processing failed"}
                        )
    ...
    def query_and_retrieve(
        self, query: str, k: int = 4, return_scores: bool = True
    ) -> List[Dict]:
        """
        查詢並檢索相關文件
    ...
    def get_rag_response(self, user_query: str, k: int = 3) -> str:
        ...
        try:
            # 檢索相關文件
            retrieved_docs = self.query_and_retrieve(
                user_query, k=k, return_scores=True
            )
    ...
    def get_processing_status(
        self, uploaded_files: List[str], upload_folder: str
    ) -> str:
        """
